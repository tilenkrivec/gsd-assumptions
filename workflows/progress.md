...
```bash
INIT_RAW=$(node /Users/tilenkrivec/.claude/get-shit-done/bin/gsd-tools.cjs init progress --include state,roadmap,project,config)
...
```bash
ROADMAP=$(node /Users/tilenkrivec/.claude/get-shit-done/bin/gsd-tools.cjs roadmap analyze)
```
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node /Users/tilenkrivec/.claude/get-shit-done/bin/gsd-tools.cjs summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node /Users/tilenkrivec/.claude/get-shit-done/bin/gsd-tools.cjs progress bar --raw)
```
...
Check discuss_mode config to show the right description:
...
`/gsd:discuss-phase {phase}` — analyze codebase, surface assumptions, capture corrections
...
**Also available:**
- `/gsd:discuss-phase {phase} --auto` — gather context then auto-advance through plan → execute
- `/gsd:plan-phase {phase}` — skip context, plan directly
...
Check discuss_mode config (already loaded from init_context or read here):
...
`/gsd:discuss-phase {Z+1}` — analyze codebase, surface assumptions, capture corrections
...
**Also available:**
- `/gsd:discuss-phase {Z+1} --auto` — gather context then auto-advance through plan → execute
- `/gsd:plan-phase {Z+1}` — skip context, plan directly
