...
@/Users/tilenkrivec/.claude/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT_RAW=$(node /Users/tilenkrivec/.claude/get-shit-done/bin/gsd-tools.cjs init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
...
```bash
PHASE_INFO=$(node /Users/tilenkrivec/.claude/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
```
...
Check discuss_mode config to determine the context-gathering option:
...
Use AskUserQuestion:
- header: "No context"
- question: "No CONTEXT.md found for Phase {X}. Plans will use research and requirements only — your design preferences won't be included. Continue or gather context first?"
- options:
  - "Continue without context" — Plan using research + requirements only
  - "Gather context (assumptions mode)" — Analyze codebase, surface assumptions, capture corrections

If "Continue without context": Proceed to step 5.
If "Gather context (assumptions mode)": Display `/gsd:discuss-phase {X}` and exit workflow. (discuss-phase will use assumptions mode automatically based on config)

**If `DISCUSS_MODE` is "discuss" (default):**
...
```bash
PHASE_DESC=$(node /Users/tilenkrivec/.claude/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node /Users/tilenkrivec/.claude/get-shit-done/bin/gsd-tools.cjs state-snapshot)
...
```
Task(
  prompt="First, read /Users/tilenkrivec/.claude/agents/gsd-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
```
Task(
  prompt="First, read /Users/tilenkrivec/.claude/agents/gsd-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
